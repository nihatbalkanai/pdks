...
class EkKazancController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->boolean('taksitli') && !empty($validated['taksit_sayisi']) && $validated['taksit_sayisi'] > 1) {
            ...
            for ($i = 1; $i <= $taksitSayisi; $i++) {
                ...
                $kayit = PersonelPrimKazanc::create([
                    'firma_id' => $firmaId,
    ...
    public function update(Request $request, $id)
    {
        ...
        $kayit = PersonelPrimKazanc::where('firma_id', $firmaId)->findOrFail($id);
    ...
    {
        $silinen = PersonelPrimKazanc::where('taksit_grup_id', $grupId)->delete();

...
const filtered = computed(() => {
    ...
    return props.personeller.filter(p => (p.kart_no || '').toLowerCase().includes(q) || (p.ad || '').toLocaleLowerCase('tr-TR').includes(q) || (p.soyad || '').toLocaleLowerCase('tr-TR').includes(q) || (p.telefon || '').includes(q));
});
...
const doSend = () => {
    ...
    axios.post(route('toplu-islemler.toplu-mesaj.gonder'), { ...form }).then(() => { clearInterval(iv); progress.value = 100; isProcessing.value = false; Swal.fire('Başarılı!', 'Mesajlar gönderildi.', 'success'); }).catch(e => Swal.fire('Hata', e.response?.data?.message || 'Hata oluştu', 'error'));
};
</script>

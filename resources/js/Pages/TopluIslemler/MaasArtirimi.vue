...
const submit = () => {
    ...
    axios.post(route('toplu-islemler.maas-artirimi.uygula'), { ...form }).then(() => { clearInterval(iv); progress.value = 100; isProcessing.value = false; Swal.fire('Başarılı!', `${selectedIds.value.length} personelin maaşı güncellendi.`, 'success'); }).catch(e => Swal.fire('Hata', e.response?.data?.message || 'Hata oluştu', 'error'));
};
</script>

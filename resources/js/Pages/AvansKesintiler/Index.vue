...
const liste = ref([...(props.kesintiler || [])]);
...
const submitForm = async () => {
    try {
        let res;
        if (editingId.value) {
            res = await axios.put(route('avans-kesintiler.update', editingId.value), { ...form });
            ...
            res = await axios.post(route('avans-kesintiler.store'), { ...form });
        }
...
const deleteRecord = () => {
    ...
    Swal.fire({ title: 'Emin misiniz?', icon: 'warning', showCancelButton: true, confirmButtonColor: '#d33', cancelButtonText: 'İptal', confirmButtonText: row?.taksit_grup_id ? 'Sadece Bu Taksiti Sil' : 'Sil',
        ...
        if (result.isConfirmed) {
            try {
                await axios.delete(route('avans-kesintiler.destroy', selectedRow.value));
            ...
            try {
                const res = await axios.delete(route('avans-kesintiler.destroy-grup', row.taksit_grup_id));
                ...
                <div v-if="showAddForm" class="px-4 py-3 bg-yellow-50 border-b border-gray-300 text-xs">
                    <div class="flex items-end gap-3 flex-wrap">
                        ...
                        <label v-if="!editingId" class="flex items-center gap-1.5 cursor-pointer self-end pb-0.5">
                            <input type="checkbox" v-model="form.taksitli" class="rounded text-blue-600 w-3.5 h-3.5" />
                        ...
                        <template v-if="form.taksitli && !editingId">
                            ...
                            <div v-if="taksitTutar > 0" class="bg-blue-100 border border-blue-300 rounded px-2 py-1 text-blue-800 font-bold text-[11px] self-end">
                    ...
                    <table class="w-full text-xs border-collapse">
                        ...
                        <tbody>
                            <tr v-for="k in liste" :key="k.id"
                                ...
                                :class="{'!bg-yellow-200 font-semibold': selectedRow === k.id, '!bg-blue-50/70': k.taksit_no && selectedRow !== k.id}">
                                ...
                                <td class="py-1 px-2 border-r border-gray-200 text-center">
                                    <span v-if="k.taksit_no" class="inline-flex items-center justify-center bg-blue-100 text-blue-700 border border-blue-300 rounded px-1.5 py-0.5 text-[10px] font-bold" title="Taksitli İşlem">

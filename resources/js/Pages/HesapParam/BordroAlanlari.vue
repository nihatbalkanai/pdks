...
const yeniAlan = () => {
    ...
    // Otomatik kod ata (son kod + 1)
    const maxKod = props.alanlar.reduce((m, a) => Math.max(m, a.kod), 0);
    form.kod = maxKod + 1;
    seciliId.value = null;
...
const kaydet = () => {
    if (form.id) {
        form.put(route('tanim.bordro-alanlari.update', form.id), {
            onSuccess: () => Swal.fire({toast:true, position:'top-end', icon:'success', title:'Güncellendi', showConfirmButton:false, timer:1500})
        });
    } else {
        form.post(route('tanim.bordro-alanlari.store'), {
            onSuccess: () => { modalAcik.value = false; Swal.fire({toast:true, position:'top-end', icon:'success', title:'Eklendi', showConfirmButton:false, timer:1500}); }
        });
    }
};
...
const sil = (id) => {
    Swal.fire({title: 'Emin misiniz?', icon: 'warning', showCancelButton: true, confirmButtonText: 'Evet, Sil'}).then((res) => {
        if(res.isConfirmed) { router.delete(route('tanim.bordro-alanlari.destroy', id), { onSuccess: () => { seciliId.value = null; } }); }
    });
};
</script>
    ...
    <div class="p-4 h-full flex flex-col">
        <div class="bg-white border border-gray-400 shadow-md flex flex-col h-full overflow-hidden">
            ...
            <div class="bg-gradient-to-r from-[#d8e4f8] to-[#c0d0e8] border-b border-gray-400 px-4 py-2 flex items-center">
                ...
                <h2 class="font-bold text-sm text-gray-800">Bordro Alanları</h2>
            </div>
                ...
                <div class="flex-1 flex flex-col">
                    <div class="flex-1 overflow-auto">
                        <table class="w-full text-sm text-left">
                            ...
                            <tbody>
                                <tr v-for="a in alanlar" :key="a.id"
                                    @click="secSatir(a)"
                                ...
                                <tr v-if="!alanlar.length"><td colspan="7" class="px-3 py-6 text-center text-gray-400">Kayıt bulunamadı.</td></tr>
                            </tbody>
                        </table>
                    </div>
                    ...
                    <div v-if="seciliId" class="border-t border-gray-300 bg-gray-50 px-4 py-3">
                        <div class="flex items-end gap-4 flex-wrap">
                            <div class="w-20"><label class="block text-xs font-bold mb-1">Kod:</label><input v-model.number="form.kod" type="number" class="w-full text-sm rounded border-gray-300 bg-yellow-50 font-bold"></div>
                            <div class="flex-1 min-w-[200px]"><label class="block text-xs font-bold mb-1">Açıklama:</label><input v-model="form.aciklama" type="text" class="w-full text-sm rounded border-gray-300"></div>
                            <div class="w-48"><label class="block text-xs font-bold mb-1">Bordro Tipi:</label>
                                <select v-model="form.bordro_tipi" class="w-full text-sm rounded border-gray-300"><option v-for="t in bordroTipleri" :key="t.value" :value="t.value">{{ t.label }}</option></select>
                            </div>
        ...
        <div class="bg-white rounded shadow-xl w-[450px] border border-gray-300 overflow-hidden">
            ...
            <div class="p-4 space-y-3">
                <div class="flex gap-3">
                    <div class="w-20"><label class="block text-xs font-bold mb-1">Kod *</label><input v-model.number="form.kod" type="number" class="w-full text-sm rounded border-gray-300"></div>
                    <div class="flex-1"><label class="block text-xs font-bold mb-1">Açıklama *</label><input v-model="form.aciklama" type="text" class="w-full text-sm rounded border-gray-300" placeholder="NORMAL ÇALIŞMA"></div>
                </div>
                <div><label class="block text-xs font-bold mb-1">Bordro Tipi</label><select v-model="form.bordro_tipi" class="w-full text-sm rounded border-gray-300"><option v-for="t in bordroTipleri" :key="t.value" :value="t.value">{{ t.label }}</option></select></div>
                <div class="flex items-center gap-6">
                    <label class="flex items-center gap-1.5"><input v-model="form.gun" type="checkbox" class="rounded text-blue-600"><span class="text-sm font-medium">Gün</span></label>
                    <label class="flex items-center gap-1.5"><input v-model="form.saat" type="checkbox" class="rounded text-blue-600"><span class="text-sm font-medium">Saat</span></label>
                    <label class="flex items-center gap-1.5"><input v-model="form.ucret" type="checkbox" class="rounded text-blue-600"><span class="text-sm font-medium">Ücret</span></label>
                </div>
            </div>
            <div class="bg-gray-50 p-3 border-t border-gray-200 flex justify-end gap-2">
                ...
                <button @click="kaydet" class="px-3 py-1.5 bg-yellow-600 text-white rounded text-sm hover:bg-yellow-700">Kaydet</button>
            </div>
        </div>
    </div>
</AuthenticatedLayout>

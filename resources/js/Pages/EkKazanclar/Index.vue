...
const submitForm = () => {
    form.post(route('ek-kazanclar.store'), {
        onSuccess: () => {
            showAddForm.value = false;
            form.reset();
            Swal.fire({ title: 'Başarılı!', text: 'Ek kazanç eklendi.', icon: 'success', timer: 1500 });
        },
        onError: () => {
...
const deleteRecord = () => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            router.delete(route('ek-kazanclar.destroy', selectedRow.value), {
                onSuccess: () => {
            ...
            <div class="bg-white border border-gray-400 shadow-md flex flex-col h-full">
                ...
                <div v-if="showAddForm" class="px-4 py-3 bg-yellow-50 border-b border-gray-300 text-xs">
                    <div class="flex items-end gap-3">
                        ...
                        <div>
                            ...
                            <select v-model="form.bordro_alani" class="border-gray-300 rounded-sm py-1 px-2 text-xs w-32">
                ...
                <div class="flex-1 overflow-auto">
                    <table class="w-full text-xs border-collapse">
                        ...
                        <tbody>
                            <tr v-for="k in kazanclar" :key="k.id"
